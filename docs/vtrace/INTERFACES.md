...
Interface type: target (greenfield). Controls SHIELD's external and cross-layer boundaries so future work packages cannot change them silently. IF-001..004 restate the `SPECIFICATION_BASELINE.md` public contracts; IF-005..006 add the crate API and CLI introduced in `ARCHITECTURE.md`. None are implemented yet.
...
| IF-005 | `shield-network` library API | API (Rust crate) | PKG-001 | PKG-002..006 | Public types/functions semver; breaking change is change-control | future VER-007 / cargo test |
| IF-006 | `shield` CLI | CLI | PKG-006 | maintainers, agents, analysts | Subcommands/flags additive (incl. `--scale`); output schemas versioned | future VER-001 / command review |

## Interface Details

### IF-001: Corpus entry file

Purpose: one healthcare delivery element as a reviewable, scored, labelled, **scale-tagged** record.

Inputs: frontmatter (`id`, `type`, `scale`, `market`, `facility_pathway_or_catchment`, `tier`, `sla`, source rows) + a dimension-score block (DIM-01..13).
...
Errors: cited quantity with no registry entry → Citation Auditor finding; HRSA/CMS/AHA/Census/CDC/Dartmouth proxy used as observed → must be labelled proxy.
...
Inputs: tier definitions, SLA terms (access time, capacity, service breadth, outcomes), element tier assignments.
...
### IF-005: `shield-network` library API

Purpose: the graph primitive every layer builds on.
Inputs: `Facility` nodes and `Referral` edges with attributes and identity, including `capacity_beds` and typed `DemandBasis` (`Surge` or `Baseline`).
Outputs: graph queries — `add_facility`, `add_referral`, `facility_count`, `referral_count`, `degree`, `is_connected`, `has_diverse_path` (redundant referral/access pathway), and `incident_capacity_beds`.
...
| IFQ-005 | Whether `has_diverse_path` remains valid for referral/catchment edges. | Defer to calibration and FIND-003. |
...
| Clinician / Care-Delivery Lead | IF-005 exposes facility/referral connectivity and capacity as typed queries with error handling, not panics; transfer semantics are calibration risk. | pass |
